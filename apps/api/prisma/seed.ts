 ...
 *
 * Todos os dados sao ficticios: nomes, cargos e o dominio `nefroclinicas.exemplo`
 * nao correspondem a pessoas ou contas reais.
 */
import { PrismaClient } from '@prisma/client'

const prisma = new PrismaClient()

const DOMINIO = 'nefroclinicas.exemplo'
...
const u = (
  ...
  oneDriveGb: number,
  arquivos: number,
): SeedUsuario => ({ nome, local, cargo, depto, skuCodigo, diasUltimoAcesso, mfa, oneDriveGb, arquivos })
...
const SERIE_TENANT = [26, 25, 27, 24, 26, 23, 25, 22, 24, 21, 23, 20, 22, 21, 19, 20, 18, 19, 17, 18]
 ...
 */
function serieDoUsuario(dias: number | null): number[] {
  if (dias === null) return Array.from({ length: 12 }, () => 0)
  const ativo = dias <= 30
...
const CONFIGURACOES = [
  ...
  { chave: 'contaConectada', valor: `rafael.nogueira@${DOMINIO}` },
  { chave: 'quotaTenantGb', valor: '1024' },
]

const EXPORTACOES = [
  { arquivo: 'usuarios-inativos-01-07-2026.xlsx', geradoPor: 'Rafael Nogueira', linhas: 38, criadoEm: new Date('2026-07-01T08:00:00Z') },
  { arquivo: 'licencas-01-07-2026.xlsx', geradoPor: 'Rafael Nogueira', linhas: 4, criadoEm: new Date('2026-07-01T08:00:00Z') },
  { arquivo: 'onedrive-14-06-2026.csv', geradoPor: 'Rafael Nogueira', linhas: 38, criadoEm: new Date('2026-06-14T16:20:00Z') },
]
...
async function main() {
  ...
  for (const usuario of USUARIOS) {
    const { local, ...resto } = usuario
    await prisma.usuario.create({
      data: {
        ...resto,
        upn: `${local}@${DOMINIO}`,
        ehRecurso: usuario.diasUltimoAcesso === null,
        acessos: {
  ...
  await prisma.exportacao.createMany({ data: EXPORTACOES })

  console.log(`Seed concluido: ${SKUS.length} planos, ${USUARIOS.length} contas, ${EXPORTACOES.length} exportacoes.`)
}
